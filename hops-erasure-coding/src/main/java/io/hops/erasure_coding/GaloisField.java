...
public class GaloisField {
  ...
  public static GaloisField getInstance(int fieldSize,
    ...
    int key =
        ((fieldSize << 16) & 0xFFFF0000) + (primitivePolynomial & 0x0000FFFF);
    GaloisField gf;
   ...
   */
  public void solveVandermondeSystem(int[] x, byte[][] y, int len,
      int dataLen) {
    for (int i = 0; i < len - 1; i++) {
      for (int j = len - 1; j > i; j--) {
    ...
    for (int i = len - 1; i >= 0; i--) {
      for (int j = i + 1; j < len; j++) {
        for (int k = 0; k < dataLen; k++) {
          y[j][k] =
              (byte) (divTable[y[j][k] & 0x000000FF][x[j] ^ x[j - i - 1]]);
        }
      }
      for (int j = i; j < len - 1; j++) {
        for (int k = 0; k < dataLen; k++) {
  ...
  public void remainder(int[] dividend, int[] divisor) {
    for (int i = dividend.length - divisor.length; i >= 0; i--) {
      int ratio =
          divTable[dividend[i + divisor.length - 1]][divisor[divisor.length -
              1]];
    ...
    for (int i = dividend.length - divisor.length; i >= 0; i--) {
      for (int j = 0; j < divisor.length; j++) {
        for (int k = 0; k < dividend[i].length; k++) {
          int ratio =
              divTable[dividend[i + divisor.length - 1][k] & 0x00FF][divisor[
                  divisor.length - 1]];
