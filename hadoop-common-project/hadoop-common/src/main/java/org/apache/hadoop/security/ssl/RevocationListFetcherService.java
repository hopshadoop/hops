...
public class RevocationListFetcherService extends AbstractService implements RevocationListFetcher {
  ...
  public void serviceStart() throws Exception {
    ...
    if (fetcherThread == null) {
      ...
      if (!ready) {
        String errorMsg = "Waited more than 30 seconds to fetch the CRL, killing the service";
        LOG.error(errorMsg);
  ...
  public void serviceStop() throws Exception {
    ...
    if (crlFetcher != null) {
      URI outputURI = crlFetcher.getOutputURI();
      if (outputURI.getScheme().equals("file")) {
        FileUtils.deleteQuietly(new File(outputURI.getPath()));
      }
    }
    super.serviceStop();
  }
