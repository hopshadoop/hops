...
public class TestGetApplicationsRequest {
    ...
    public void testGetApplicationsRequest() {
        ...
        Assert.assertEquals(
                "ApplicationStates from proto is not the same with the original request",
                requestFromProto.getApplicationStates(), appStates);

        Assert.assertEquals(
                "ApplicationTags from proto is not the same with the original request",
                requestFromProto.getApplicationTags(), tags);

        Assert.assertEquals(
                "ApplicationTypes from proto is not the same with the original request",
                requestFromProto.getApplicationTypes(), types);

        Assert.assertEquals(
                "StartRange from proto is not the same with the original request",
                requestFromProto.getStartRange(), new LongRange(startBegin, startEnd));

        Assert.assertEquals(
                "FinishRange from proto is not the same with the original request",
                requestFromProto.getFinishRange(), new LongRange(finishBegin, finishEnd));

        Assert.assertEquals(
                "Limit from proto is not the same with the original request",
                requestFromProto.getLimit(), limit);

        Assert.assertEquals(
                "Queues from proto is not the same with the original request",
                requestFromProto.getQueues(), queues);

        Assert.assertEquals(
                "Users from proto is not the same with the original request",
