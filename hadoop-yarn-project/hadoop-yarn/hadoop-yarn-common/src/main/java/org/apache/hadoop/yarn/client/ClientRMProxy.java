...
public class ClientRMProxy<T> extends RMProxy<T> {
  ...
  private interface ClientRMProtocols
      extends ApplicationClientProtocol, ApplicationMasterProtocol,
      ResourceManagerAdministrationProtocol, GroupMembership {
    // Add nothing
  }
   ...
   * @param protocol
   *     Client protocol for which proxy is being requested.
   * @param <T>
   *     Type of proxy.
  ...
  public static <T> T createRMProxy(final Configuration configuration,
    ...
    return createRMProxy(configuration, protocol, INSTANCE, false);
  }
  ...
  protected void checkAllowedProtocols(Class<?> protocol) {
    Preconditions
        .checkArgument(protocol.isAssignableFrom(ClientRMProtocols.class),
            "RM does not support this client protocol");
  }

  /**
   * Get the token service name to be used for RMDelegationToken. Depending on
   * whether HA is enabled or not, this method generates the appropriate
  ...
  public static Text getRMDelegationTokenService(Configuration conf) {
    if (HAUtil.isHAEnabled(conf)) {
      ...
      for (String rmId : HAUtil.getRMHAIds(conf)) {
        ...
        services.add(SecurityUtil.buildTokenService(yarnConf
