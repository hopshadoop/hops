...
public class RpcServerFactoryPBImpl implements RpcServerFactory {
  ...
  private static final Log LOG =
      LogFactory.getLog(RpcServerFactoryPBImpl.class);
  ...
  public Server getServer(Class<?> protocol, Object instance,
    ...
    if (constructor == null) {
      ...
      try {
        pbServiceImplClazz =
            localConf.getClassByName(getPbServiceImplClassName(protocol));
      } catch (ClassNotFoundException e) {
        throw new YarnRuntimeException(
            "Failed to load class: [" + getPbServiceImplClassName(protocol) +
                "]", e);
      }
      try {
    ...
    if (method == null) {
      ...
      try {
        ...
        throw new YarnRuntimeException(
            "Failed to load class: [" + getProtoClassName(protocol) + "]", e);
      }
      try {
  ...
  private Server createServer(Class<?> pbProtocol, InetSocketAddress addr,
      Configuration conf,
      SecretManager<? extends TokenIdentifier> secretManager, int numHandlers,
    ...
    RPC.Server server = new RPC.Builder(conf).setProtocol(pbProtocol)
        ...
        .build();
    LOG.info(
        "Adding protocol " + pbProtocol.getCanonicalName() + " to the server");
    server.addProtocol(RPC.RpcKind.RPC_PROTOCOL_BUFFER, pbProtocol,
        blockingService);
    return server;
  }
}
