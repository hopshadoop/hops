...
public class HamletGen {
   ...
   *
   * @param specClass
   *     holds hamlet interfaces. e.g. {@link HamletSpec}
   * @param implClass
   *     a generic hamlet implementation. e.g. {@link HamletImpl}
   * @param outputName
   *     name of the output class. e.g. {@link Hamlet}
   * @param outputPkg
   *     package name of the output class.
   * @throws IOException
   */
  public void generate(Class<?> specClass, Class<?> implClass,
    ...
    LOG.info("Generating {} using {} and {}",
        new Object[]{outputName, specClass, implClass});
  ...
  void genFactoryMethod(String retName, String methodName, int indent) {
    puts(indent, "\n", "private <T extends _> ", retName, "<T> ", methodName,
        ...
        retName.toLowerCase(Locale.US), "\", e, opt(",
  ...
  void genNewElementMethod(String className, Method method, int indent) {
    ...
    echo(indent, "\n", "@Override\n", "public ", retName, "<", className,
        topMode ? "> " : "<T>> ", methodName, "(");
    if (params.length == 0) {
      puts(0, ") {");
      puts(indent, topMode ? "" : "  closeAttrs();\n", "  return ",
          retName.toLowerCase(Locale.US), "_(this, ",
  ...
  void genCurElementMethod(String className, Method method, int indent) {
    ...
    if (params.length == 0) {
      ...
      if (methodName.equals("base")) {
        ...
        puts(0, "String selector) {");
        puts(indent, "  return setSelector(", methodName,
            "(), selector)._();\n", "}");
      ...
      if (methodName.equals("a")) {
        ...
        puts(indent, "  return setSelector(a(), selector)",
