...
public class FSDownload implements Callable<Path> {
   ...
   */
  public static CacheLoader<Path, Future<FileStatus>> createStatusCacheLoader(
  ...
  private static boolean checkPublicPermsForAll(FileSystem fs,
      FileStatus status, FsAction dir, FsAction file) throws IOException {
  ...
  private Path copy(Path sCopy, Path dstdir) throws IOException {
    ...
    FileUtil
        .copy(sourceFs, sStat, FileSystem.getLocal(conf), dCopy, false, true,
            conf);
    return dCopy;
  }

  private long unpack(File localrsrc, File dst, Pattern pattern)
      throws IOException {
        ...
        if (lowerDst.endsWith(".jar")) {
          ...
          if (!localrsrc.renameTo(dst)) {
            throw new IOException(
                "Unable to rename file: [" + localrsrc + "] to [" + dst + "]");
          }
        }
      }
      break;
      case PATTERN: {
        ...
        if (lowerDst.endsWith(".jar")) {
          ...
          if (!localrsrc.renameTo(newDst)) {
            throw new IOException(
                "Unable to rename file: [" + localrsrc + "] to [" + newDst +
                    "]");
          }
        } else if (lowerDst.endsWith(".zip")) {
          LOG.warn(
              "Treating [" + localrsrc + "] as an archive even though it " +
