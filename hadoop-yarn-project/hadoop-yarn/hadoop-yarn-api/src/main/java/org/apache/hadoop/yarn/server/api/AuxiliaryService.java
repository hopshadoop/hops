...
public abstract class AuxiliaryService extends AbstractService {
   ...
   *
   * @param initAppContext
   *     context for the application's initialization
   */
   ...
   * that applications can access the service remotely. This will only be
   * called
   * after the service's {@link #start()} method has finished. the result may
   * be
   * cached.
   * <p/>
   * <p>
   * The information is passed along to applications via
   * {@link StartContainersResponse#getAllServicesMetaData()} that is returned
   * by
