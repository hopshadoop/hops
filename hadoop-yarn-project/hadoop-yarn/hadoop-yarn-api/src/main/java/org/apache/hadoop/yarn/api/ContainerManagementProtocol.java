...
public interface ContainerManagementProtocol {
   ...
   * <p/>
   * <p>
   * The <code>ApplicationMaster</code> has to provide details such as
   * allocated
   ...
   * containerId-to-exception map for each failed {@link StartContainerRequest}
   * in
   ...
   * <p>
   * The <code>NodeManager</code> sends a response via
   * {@link StopContainersResponse} which includes a list of {@link
   * ContainerId}
   ...
   * container. Note: None-container-specific exceptions will still be thrown
   * by
   ...
   * {@link GetContainerStatusesRequest} which includes the {@link
   * ContainerId}s
   ...
   * containerId-to-exception map for each failed request in which the
   * exception
