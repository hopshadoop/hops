...
public class TestNMClient {
  ...
  public void setup() throws YarnException, IOException {
    ...
    // start am rm client
    rmClient = (AMRMClientImpl<ContainerRequest>) AMRMClient
  ...
  private Set<Container> allocateContainers(
    ...
    int containersRequestedAny =
        rmClient.remoteRequestsTable.get(priority).get(ResourceRequest.ANY)
    ...
    while (allocatedContainerCount < containersRequestedAny &&
      ...
      if (!allocResponse.getNMTokens().isEmpty()) {
        for (NMToken token : allocResponse.getNMTokens()) {
          rmClient.getNMTokenCache()
              .setToken(token.getNodeId().toString(), token.getToken());
        }
      }
  ...
  private void testContainerManagement(NMClientImpl nmClient,
    ...
    for (Container container : containers) {
      ...
      try {
        ...
        if (!e.getMessage().contains("is not handled by this NodeManager")) {
          throw (AssertionError) (new AssertionError(
              "Exception is not expected: " + e).initCause(e));
        }
      }
      ...
      ByteBuffer securityTokens = ByteBuffer.wrap(dob.getData(), 0, dob.
          getLength());
      ...
      if (++i < size) {
        ...
        try {
          ...
          testGetContainerStatus(container, i, ContainerState.COMPLETE,
              ...
              Arrays.asList(new Integer[]{ContainerExitStatus.KILLED_BY_APPMASTER}));
        } catch (YarnException e) {
