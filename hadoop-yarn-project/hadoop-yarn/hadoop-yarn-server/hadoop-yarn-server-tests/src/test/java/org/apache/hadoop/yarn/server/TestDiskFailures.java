...
/**
 * Verify if NodeManager's in-memory good local dirs list and good log dirs
 * list
...
public class TestDiskFailures {
  ...
  private static final File testDir =
      new File("target", TestDiskFailures.class.getName()).getAbsoluteFile();
  private static final File localFSDirBase =
      new File(testDir, TestDiskFailures.class.getName() + "-localDir");
  ...
  private void testDirsFailures(boolean localORLogDirs) throws IOException {
    ...
    yarnCluster =
        new MiniYARNCluster(TestDiskFailures.class.getName(), 1, numLocalDirs,
            numLogDirs);
    ...
    List<String> list =
        localORLogDirs ? dirsHandler.getLocalDirs() : dirsHandler.getLogDirs();
  ...
  private void verifyDisksHealth(boolean localORLogDirs, String expectedDirs,
    ...
    Assert.assertEquals("Node's health in terms of disks is wrong", isHealthy,
        dirsHandler.areDisksHealthy());
    for (int i = 0; i < 10; i++) {
      Iterator<RMNode> iter =
          yarnCluster.getResourceManager().getRMContext().getActiveRMNodes()
              .values().iterator();
      if (iter.hasNext() && 
   ...
   * This would cause failure of creation of directory in
   * DiskChecker.checkDir()
   * with the same name.
   *
   * @param dir
   *     the directory to be failed
   * @throws IOException
   */
