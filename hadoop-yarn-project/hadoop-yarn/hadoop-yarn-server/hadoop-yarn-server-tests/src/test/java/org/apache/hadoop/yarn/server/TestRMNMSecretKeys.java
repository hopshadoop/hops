...
public class TestRMNMSecretKeys {
  ...
  private void validateRMNMKeyExchange(YarnConfiguration conf)
    ...
    Assert.assertNotNull(
        containerToken + "Registration should cause a key-update!",
    ...
    Assert.assertEquals(
        nmToken + "Roll-over should have incremented the key-id only by one!",
    ...
    response = nm.nodeHeartbeat(true);
    Assert.assertNull(
        containerToken + "Activation shouldn't cause any key updates!",
        response.getContainerTokenMasterKey());
    Assert.assertNull(nmToken + "Activation shouldn't cause any key updates!",
        response.getNMTokenMasterKey());
    dispatcher.await();
