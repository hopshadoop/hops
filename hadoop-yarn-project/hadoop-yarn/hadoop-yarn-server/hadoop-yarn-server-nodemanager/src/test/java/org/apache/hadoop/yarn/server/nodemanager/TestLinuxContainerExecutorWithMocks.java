...
public class TestLinuxContainerExecutorWithMocks {
  ...
  private static final Log LOG =
      LogFactory.getLog(TestLinuxContainerExecutorWithMocks.class);
  ...
  private List<String> readMockParams() throws IOException {
    ...
    LineNumberReader reader =
        new LineNumberReader(new FileReader(mockParamFile));
    String line;
  ...
  public void testContainerLaunch() throws IOException {
    ...
    assertEquals(0, ret);
    assertEquals(Arrays
            .asList(YarnConfiguration.DEFAULT_NM_NONSECURE_MODE_LOCAL_USER,
                appSubmitter, cmd, appId, containerId, workDir.toString(),
                "/bin/echo", "/dev/null", pidFile.toString(),
  ...
  public void testStartLocalizer() throws IOException {
    ...
    InetSocketAddress address =
        InetSocketAddress.createUnresolved("localhost", 8040);
    Path nmPrivateCTokensPath = new Path("file:///bin/nmPrivateCTokensPath");

    try {
      mockExec.startLocalizer(nmPrivateCTokensPath, address, "test",
          "application_0", "12345", dirsHandler.getLocalDirs(),
  ...
  public void testContainerLaunchError() throws IOException {
    ...
    File f =
        new File("./src/test/resources/mock-container-executer-with-error");
    if (!FileUtil.canExecute(f)) {
      FileUtil.setExecutable(f, true);
    }
    String executorPath = f.getAbsolutePath();
  ...
  public void testContainerKill() throws IOException {
    String appSubmitter = "nobody";
    String cmd = String
        .valueOf(LinuxContainerExecutor.Commands.SIGNAL_CONTAINER.getValue());
  ...
  public void testDeleteAsUser() throws IOException {
    String appSubmitter = "nobody";
    String cmd = String
        .valueOf(LinuxContainerExecutor.Commands.DELETE_AS_USER.getValue());
