...
public class TestDefaultContainerExecutor {
  ...
  */
  ...
  private static final Path BASE_TMP_PATH =
      new Path("target", TestDefaultContainerExecutor.class.getSimpleName());
  ...
  public void testDirPermissions() throws Exception {
    ...
    final FsPermission appCachePerm =
        new FsPermission(DefaultContainerExecutor.APPCACHE_PERM);
    final FsPermission fileCachePerm =
        new FsPermission(DefaultContainerExecutor.FILECACHE_PERM);
    final FsPermission appDirPerm =
        new FsPermission(DefaultContainerExecutor.APPDIR_PERM);
    final FsPermission logDirPerm =
        new FsPermission(DefaultContainerExecutor.LOGDIR_PERM);
    ...
    try {
      ...
      for (String dir : localDirs) {
        Path userCachePath =
            new Path(new Path(dir, ContainerLocalizer.USERCACHE), user);
        Path appCachePath =
            new Path(userCachePath, ContainerLocalizer.APPCACHE);
