...
class DefaultSchedulerPage extends RmView {
  ...
  static class QueueInfoBlock extends HtmlBlock {
    ...
    @Inject
    QueueInfoBlock(RMContext context, ViewContext ctx, ResourceManager rm) {
      super(ctx);
    ...
    public void render(Block html) {
      info("\'" + sinfo.getQueueName() + "\' Queue Status").
          ...
          _("Minimum Queue Memory Capacity:",
              Integer.toString(sinfo.getMinQueueMemoryCapacity())).
          _("Maximum Queue Memory Capacity:",
              Integer.toString(sinfo.getMaxQueueMemoryCapacity())).
          _("Number of Nodes:", Integer.toString(sinfo.getNumNodes())).
          _("Used Node Capacity:",
              Integer.toString(sinfo.getUsedNodeCapacity())).
          _("Available Node Capacity:",
              Integer.toString(sinfo.getAvailNodeCapacity())).
          _("Total Node Capacity:",
              Integer.toString(sinfo.getTotalNodeCapacity())).
          _("Number of Node Containers:",
              Integer.toString(sinfo.getNumContainers()));
    ...
    public void render(Block html) {
      ...
      if (fs == null) {
        ...
        ul.
            ...
            span().$style(Q_END)._("100%")._().
            span().$style(
            join(width(delta), ';', used > set ? OVER : UNDER, ';',
