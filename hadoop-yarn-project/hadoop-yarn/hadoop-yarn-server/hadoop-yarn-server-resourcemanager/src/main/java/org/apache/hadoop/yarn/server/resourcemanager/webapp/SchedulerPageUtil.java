    ...
    private void storeExpandedQueue(Block html) {
      html.
          ...
          _("function storeExpandedQueue(e, data) {",
              ...
              "  var ACTION_OPEN = 'open';", "  var ACTION_CLOSED = 'closed';",
              "  var $li = $(data.args[0]);",
              "  var action = ACTION_CLOSED;  //closed or open",
              "  var queueName = ''", "  if ($li.hasClass('jstree-open')) {",
              "      action=ACTION_OPEN;", "  }",
              ...
              "  var currentParam = window.location.href.split('?');",
              "  var tmpCurrentParam = currentParam;",
              "  var queryString = '';", "  if (tmpCurrentParam.length > 1) {",
              "    // openQueues=q1#q2&param1=value1&param2=value2",
              "    tmpCurrentParam = tmpCurrentParam[1];",
              ...
              "    else {", "      tmpCurrentParam=tmpCurrentParam[0];",
              ...
              "        }", "      }", "    }", "  } else {",
              "    if (action == ACTION_OPEN) {", "      tmpCurrentParam = '';",
              "      currentParam = tmpCurrentParam;",
              ...
              "  if (index != -1) {", "    // removing openQueues=",
              "    var tmp = queryString.substr(11, queryString.length);",
              "    tmp = tmp.split('#');", "    var len = tmp.length;",
              "    var newQueryString = '';",
              "    for (var i = 0 ; i < len ; i++) {",
              "      if (tmp[i] != queueName) {",
              "        if (newQueryString != '') {",
