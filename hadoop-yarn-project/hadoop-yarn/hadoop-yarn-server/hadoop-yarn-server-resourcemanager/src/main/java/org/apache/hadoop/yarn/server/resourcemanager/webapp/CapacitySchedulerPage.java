...
class CapacitySchedulerPage extends RmView {
  ...
  static final String Q_GIVEN =
      "left:0%;background:none;border:1px dashed rgba(0,0,0,0.25)";
  ...
  static class LeafQueueInfoBlock extends HtmlBlock {
    ...
    @Inject
    LeafQueueInfoBlock(ViewContext ctx, CSQInfo info) {
      super(ctx);
      lqinfo = (CapacitySchedulerLeafQueueInfo) info.qinfo;
    }

    //Return a string describing one resource as a percentage of another
    private String getPercentage(ResourceInfo numerator,
        ResourceInfo denominator) {
      ...
      if (denominator.getMemory() != 0) {
        percentString.append(" (<span title='of used resources in this queue'>")
            .append(StringUtils.format("%.2f",
                numerator.getMemory() * 100.0 / denominator.getMemory()) +
                "%</span>)");
      }
      ...
      if (denominator.getvCores() != 0) {
        percentString.append(" (<span title='of used resources in this queue'>")
            .append(StringUtils.format("%.2f",
                numerator.getvCores() * 100.0 / denominator.getvCores()) +
                "%</span>)");
      }
    ...
    protected void render(Block html) {
      ...
      for (UserInfo entry : users) {
        activeUserList.append(entry.getUsername()).append(" &lt;")
            ...
            .append(
                ", Non-Schedulable Apps: " + entry.getNumPendingApplications())
      ...
      ResponseInfo ri =
          info("\'" + lqinfo.getQueuePath().substring(5) + "\' Queue Status").
              ...
              _("Max Schedulable Applications:",
                  Integer.toString(lqinfo.getMaxActiveApplications())).
              _("Max Schedulable Applications Per User:",
                  Integer.toString(lqinfo.getMaxActiveApplicationsPerUser())).
              _("Configured Capacity:", percent(lqinfo.getCapacity() / 100)).
              _("Configured Max Capacity:",
                  percent(lqinfo.getMaxCapacity() / 100)).
              _("Configured Minimum User Limit Percent:",
                  Integer.toString(lqinfo.getUserLimit()) + "%").
              _("Configured User Limit Factor:",
                  String.format("%.1f", lqinfo.getUserLimitFactor())).
    ...
    public void render(Block html) {
      ...
      for (CapacitySchedulerQueueInfo info : subQueues) {
        ...
        LI<UL<Hamlet>> li = ul.
            ...
            span()
            .$style(join(Q_GIVEN, ";font-size:1px;", width(absCap / absMaxCap)))
            .
                _('.')._().
                span().$style(
                join(width(absUsedCap / absMaxCap), ";font-size:1px;left:0%;",
                    absUsedCap > absCap ? Q_OVER : Q_UNDER)).
                _('.')._().
    ...
    public void render(Block html) {
      ...
      if (cs == null) {
        ...
        ul.
            ...
            a(_Q).$style(width(Q_MAX_WIDTH)).
            span()
            .$style(join(width(used), ";left:0%;", used > 1 ? Q_OVER : Q_UNDER))
            ._(".")._().
            span(".q", "root")._().
            span().$class("qstats").$style(left(Q_STATS_POS)).
            _(join(percent(used), " used"))._().
  ...
  protected void postHead(Page.HTML<_> html) {
    html.
        ...
        _("$(function() {",
            ...
            "    jstree({", "    core: { animation: 188, html_titles: true },",
