...
public class ServerRMProxy<T> extends RMProxy<T> {
   ...
   * @param protocol
   *     Server protocol for which proxy is being requested.
   * @param <T>
   *     Type of proxy.
  ...
  public static <T> T createRMProxy(final Configuration configuration,
      final Class<T> protocol, boolean leader) throws IOException {
    return createRMProxy(configuration, protocol, INSTANCE, leader);
  }
  ...
  protected void checkAllowedProtocols(Class<?> protocol) {
    Preconditions
        .checkArgument(protocol.isAssignableFrom(ResourceTracker.class),
