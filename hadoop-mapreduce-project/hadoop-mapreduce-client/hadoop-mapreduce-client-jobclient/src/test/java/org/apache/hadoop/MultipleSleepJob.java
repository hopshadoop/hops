 ...
 * some disk space.
 */
public class MultipleSleepJob extends Configured implements Tool {
  private static final Log LOG = LogFactory.getLog(MultipleSleepJob.class);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new Configuration(), new MultipleSleepJob(), args);
    System.exit(res);
  }
  ...
  public Job createJob(int numMapper, int numReducer, 
    ...
    job.setJarByClass(MultipleSleepJob.class);
    job.setNumReduceTasks(numReducer);
  ...
  public int run(String[] args) throws Exception {
    ...
    if(args.length < 1) {
      System.err.println("SleepJob [-m numMapper] [-r numReducer]" +
          ...
          " [-recordt recordSleepTime (msec)] [-nj number of jobs]");
      ToolRunner.printGenericCommandUsage(System.err);
      return 2;
    }

    int numMapper = 1, numReducer = 1, nj=1;
    ...
    for(int i=0; i < args.length; i++ ) {
      if(args[i].equals("-m")) {
        ...
        nj = Integer.parseInt(args[++i]);
      }
    }
    
    // sleep for *SleepTime duration in Task by recSleepTime per record
    mapSleepCount = (int) Math.ceil(mapSleepTime / ((double) recSleepTime));
    reduceSleepCount = (int) Math.
            ceil(reduceSleepTime / ((double) recSleepTime));
    ArrayList<Job> jobs = new ArrayList<Job>(nj);
    for (int i = 0; i < nj; i++) {
    ...
    int result = 0;
    for(Job job: jobs){
      result = job.monitorAndPrintJob()? 0 : 1;
      if(result ==1){
        break;
      }
    }
