...
public class TestReadWhileWriting {
  ...
  public void pipeline_02_03() throws Exception {
    ...
    try {
      ...
      {
        final FSDataOutputStream out = fs.create(p, true, fs.getConf()
      ...
      //c. On M1, append another half block of data.  Close file on M1.
      {
        ...
        final DistributedFileSystem dfs =
            ugi.doAs(new PrivilegedExceptionAction<DistributedFileSystem>() {
                  @Override
  ...
  private static FSDataOutputStream append(FileSystem fs, Path p)
    ...
    for (int i = 0; i < 10; i++) {
      try {
        ...
        if (re.getClassName().equals(
            RecoveryInProgressException.NonAbortingRecoveryInProgressException.class
                .getName())) {
          AppendTestUtil.LOG
              .info("Will sleep and retry, i=" + i + ", p=" + p, re);
          Thread.sleep(1000);
        } else {
          throw re;
        }
