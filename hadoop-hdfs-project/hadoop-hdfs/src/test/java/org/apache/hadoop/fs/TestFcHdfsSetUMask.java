...
public class TestFcHdfsSetUMask {
  ...
  private static final FsPermission BLANK_PERMISSIONS =
      FsPermission.createImmutable((short) 0000);
  ...
  private static final FsPermission BLANK_TEST_UMASK =
      FsPermission.createImmutable((short) (0000 ^ 0777));
  ...
  private static final FsPermission WIDE_OPEN_TEST_UMASK =
      FsPermission.createImmutable((short) (0777 ^ 0777));
  ...
  public void setUp() throws Exception {
    ...
    fc.mkdir(fileContextTestHelper.getTestRootPath(fc),
  ...
  public void testMkdirWithExistingDir(FsPermission umask,
    ...
    Assert.assertEquals("permissions on directory are wrong", expectedPerms,
  ...
  public void testCreateRecursiveWithExistingDir(FsPermission umask,
    ...
    Assert.assertEquals("permissions on file are wrong", expectedPerms,
        fc.getFileStatus(f).getPermission());
  }
  ...
  public void testCreateRecursiveWithNonExistingDir(FsPermission umask,
    ...
    createFile(fc, f);
    Assert.assertTrue(isFile(fc, f));
    Assert.assertEquals("permissions on file are wrong", expectedFilePerms,
        fc.getFileStatus(f).getPermission());
    Assert.assertEquals("permissions on parent directory are wrong",
