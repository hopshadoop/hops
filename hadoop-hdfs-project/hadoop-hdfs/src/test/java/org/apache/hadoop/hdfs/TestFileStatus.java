...
public class TestFileStatus {
  ...
  private static void writeFile(FileSystem fileSys, Path name, int repl,
    ...
    FSDataOutputStream stm = fileSys
        .create(name, true, HdfsConstants.IO_FILE_BUFFER_SIZE, (short) repl,
            (long) blockSize);
  ...
  public void testGetFileStatusOnDir() throws Exception {
    ...
    assertEquals(dir + " should be zero size ", 0,
        fs.getContentSummary(dir).getLength());
    assertEquals(dir + " should be zero size using hftp", 0,
        hftpfs.getContentSummary(dir).getLength());
    ...
    assertEquals(dir + " size should be " + expected, expected,
        fs.getContentSummary(dir).getLength());
    assertEquals(dir + " size should be " + expected + " using hftp", expected,
        hftpfs.getContentSummary(dir).getLength());
