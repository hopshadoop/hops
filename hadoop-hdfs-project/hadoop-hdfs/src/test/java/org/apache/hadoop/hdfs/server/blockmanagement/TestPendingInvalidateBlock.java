...
public class TestPendingInvalidateBlock {
  ...
  public void testPendingDeleteUnknownBlocks() throws Exception {
    ...
    //the 4 blocks added during the restart are still in the DB
    Assert.assertEquals(4L, cluster.getNamesystem().getPendingDeletionBlocks()); 
    // restart DataNodes
    ...
    Assert.assertEquals(6, cluster.getNamesystem().getPendingDeletionBlocks()); 
