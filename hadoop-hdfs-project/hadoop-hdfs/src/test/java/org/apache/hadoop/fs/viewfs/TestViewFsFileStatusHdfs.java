...
public class TestViewFsFileStatusHdfs {
  ...
  public void testGetFileChecksum() throws IOException, URISyntaxException {
    ...
    fileSystemTestHelper.createFile(fHdfs,
        fileSystemTestHelper.getTestRootPath(fHdfs, someFile + "other"), 1,
        512);
    // Get checksum through ViewFS
    FileChecksum viewFSCheckSum =
        vfs.getFileChecksum(new Path("/vfstmp/someFileForTestGetFileChecksum"));
    ...
    FileChecksum otherHdfsFileCheckSum =
        fHdfs.getFileChecksum(new Path(someFile + "other"));
    ...
    assertFalse("Some other HDFS file which should not have had the same " +
        ...
        viewFSCheckSum.equals(otherHdfsFileCheckSum));
  }
