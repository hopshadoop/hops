...
public class BlockReaderFactory {
  ...
  throws IOException {
    ...
    if (conf.useLegacyBlockReader) {
      ...
      RemoteBlockReader reader = RemoteBlockReader.newBlockReader(file,
          ...
          verifyChecksum, clientName, peer, datanodeID, peerCache, cachingStrategy);
      return reader;
    } else {
    ...
    case SUCCESS:
      ...
      try {
        reader = new BlockReaderLocal(conf, file, block,
            startOffset, len, fis[0], fis[1], datanodeID, verifyChecksum, fisCache);
      } finally {
        if (reader == null) {
   ...
   */
  static BlockReader getLegacyBlockReaderLocal(DFSClient dfsClient, String src, ExtendedBlock blk,
