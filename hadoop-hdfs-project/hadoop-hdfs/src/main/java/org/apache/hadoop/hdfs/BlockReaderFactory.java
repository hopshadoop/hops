...
public class BlockReaderFactory implements ShortCircuitReplicaCreator {
  ...
  public BlockReaderFactory setEmulateHdfsClient(Boolean emulateHdfsClient){
    this.emulateHdfsClient = emulateHdfsClient;
    return this;
  }
  
  public BlockReaderFactory setLocatedBlock(LocatedBlock locatedBlock) {
    this.locatedBlock = locatedBlock;
    return setExtendedBlock(locatedBlock.getBlock());
  }

  public BlockReaderFactory setExtendedBlock(ExtendedBlock block) {
    this.block = block;
    return this;
  }
  ...
  public BlockReader build() throws IOException {
    ...
    if (locatedBlock!=null && locatedBlock.isPhantomBlock() && !emulateHdfsClient) {
      DFSClient.LOG.debug("Stuffed Inode:  Found Phantom LocatedBlock. Initializing BlockReaderDB, Data Len: "
          + locatedBlock.getData().length);
      return new BlockReaderDB(locatedBlock.getData(), (int) startOffset); //small file. we can safely cast long to int
    }
  ...
  private BlockReader getLegacyBlockReaderLocal() throws IOException {
    ...
    try {
      return BlockReaderLocalLegacy.newBlockReader(conf,
          ...
          datanode, startOffset, length, tracer);
