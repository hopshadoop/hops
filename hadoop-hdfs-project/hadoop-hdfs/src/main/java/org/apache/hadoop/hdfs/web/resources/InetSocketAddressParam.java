    ...
    InetSocketAddress parse(final String str) {
      if (str == null) {
        throw new IllegalArgumentException(
            "The input string is null: expect " + getDomain());
      }
      final int i = str.indexOf(':');
      if (i < 0) {
        throw new IllegalArgumentException(
            "Failed to parse \"" + str + "\" as " + getDomain() +
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Failed to parse \"" + str + "\" as " + getDomain() +
                ": the ':' position is " + i + " but failed to parse PORT.", e);
      }
      ...
      try {
        ...
        throw new IllegalArgumentException("Failed to parse \"" + str +
            "\": cannot create InetSocketAddress(host=" + host + ", port=" +
            port + ")", e);
      }
    }
