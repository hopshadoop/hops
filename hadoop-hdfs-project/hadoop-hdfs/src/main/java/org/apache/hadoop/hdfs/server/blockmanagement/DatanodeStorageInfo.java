...
public class DatanodeStorageInfo {
  ...
  public Map<Long, Integer> getAllStorageReplicas(int numBuckets, int nbThreads, int bucketPerThread) throws IOException {
    return DatanodeStorageInfo.getAllStorageReplicas(numBuckets, sid, nbThreads, bucketPerThread);
  }
  
  public static Map<Long, Integer> getAllStorageReplicas(int numBuckets, final int sid, int nbThreads, int bucketPerThread)
      throws IOException {
    final Map<Long, Integer> result = new ConcurrentHashMap<>();
    ...
    try {
      Slicer.slice(numBuckets, bucketPerThread, nbThreads, new Slicer.OperationHandler() {
        ...
        public void handle(final int startIndex, final int endIndex) throws Exception {
          ...
          result.putAll((Map<Long, Integer>) findBlocksHandler.handle());
        }
      });
    } catch (Exception e) {
