...
public class DatanodeJspHelper {
  ...
  static void generateDirectoryStructure(JspWriter out, HttpServletRequest req,
      HttpServletResponse resp, Configuration conf)
      throws IOException, InterruptedException {
    final String dir = JspHelper
        .validatePath(StringEscapeUtils.unescapeHtml(req.getParameter("dir")));
    if (dir == null) {
    ...
    if (targetStatus == null) { // not exists
      out.print(
          "<h3>File or directory : " + StringEscapeUtils.escapeHtml(target) +
              " does not exist</h3>");
      JspHelper
          .printGotoForm(out, namenodeInfoPort, tokenString, target, nnAddr);
    } else {
      if (!targetStatus.isDir()) { // a file
        ...
        if (locations == null || locations.length == 0) {
          ...
          String redirectLocation = HttpConfig2.getSchemePrefix() + fqdn + ":" +
      ...
      String[] headings = {"Name", "Type", "Size", "Replication", "Block Size",
          "Modification Time", "Permission", "Owner", "Group"};
      out.print("<h3>Contents of directory ");
      JspHelper
          .printPathWithLinks(dir, out, namenodeInfoPort, tokenString, nnAddr);
      out.print("</h3><hr>");
    ...
    out.print("<br><a href=\"" + HttpConfig2.getSchemePrefix() +
  ...
  static void generateFileDetails(JspWriter out, HttpServletRequest req,
    ...
    final String nnAddr = StringEscapeUtils
        .escapeHtml(req.getParameter(JspHelper.NAMENODE_ADDRESS));
    if (nnAddr == null) {
      out.print(JspHelper.NAMENODE_ADDRESS + " url param is null");
      return;
    }

    final int chunkSizeToView = JspHelper
        .string2ChunkSizeToView(req.getParameter("chunkSizeToView"),
    ...
    List<LocatedBlock> blocks =
        dfs.getNamenode().getBlockLocations(filename, 0, Long.MAX_VALUE)
            .getLocatedBlocks();
    // Add the various links for looking at the file contents
    // URL for downloading the full file
    String downloadUrl =
        HttpConfig2.getSchemePrefix() + req.getServerName() + ":" +
    ...
    String fqdn = canonicalize(chosenNode.getIpAddr());
    String tailUrl =
        HttpConfig2.getSchemePrefix() + fqdn + ":" + chosenNode.getInfoPort() +
            "/tail.jsp?filename=" + URLEncoder.encode(filename, "UTF-8") +
            "&namenodeInfoPort=" + namenodeInfoPort + "&chunkSizeToView=" +
            chunkSizeToView +
            JspHelper.getDelegationTokenUrlParam(tokenString) +
            JspHelper.getUrlParam(JspHelper.NAMENODE_ADDRESS, nnAddr) +
            "&referrer=" + URLEncoder
            .encode(req.getRequestURL() + "?" + req.getQueryString(), "UTF-8");
    out.print("<a href=\"" + tailUrl + "\">Tail this file</a><br>");

    out.print("<form action=\"/browseBlock.jsp\" method=GET>");
    out.print(
        "<b>Chunk size to view (in bytes, up to file's DFS block size): </b>");
    out.print(
        "<input type=\"hidden\" name=\"blockId\" value=\"" + blockId + "\">");
    out.print("<input type=\"hidden\" name=\"blockSize\" value=\"" + blockSize +
        "\">");
    out.print(
        "<input type=\"hidden\" name=\"startOffset\" value=\"" + startOffset +
            "\">");
    out.print(
        "<input type=\"hidden\" name=\"filename\" value=\"" + filename + "\">");
    out.print(
        "<input type=\"hidden\" name=\"genstamp\" value=\"" + genStamp + "\">");
    out.print(
        "<input type=\"hidden\" name=\"datanodePort\" value=\"" + datanodePort +
            "\">");
      ...
      for (DatanodeInfo loc : locs) {
        ...
        String blockUrl =
            HttpConfig2.getSchemePrefix() + fqdn + ":" + loc.getInfoPort() +
                ...
                datanodePort + "&genstamp=" +
                cur.getBlock().getGenerationStamp() + "&namenodeInfoPort=" +
        ...
        String blockInfoUrl =
            HttpConfig2.getSchemePrefix() + nnCanonicalName + ":" +
                ...
                blockidstring;
        out.print(
            "<td>&nbsp</td><td><a href=\"" + blockUrl + "\">" + datanodeAddr +
                "</a></td><td>" + "<a href=\"" + blockInfoUrl +
    ...
    out.print("<hr>");
    out.print(
        "<br><a href=\"" + HttpConfig2.getSchemePrefix() + nnCanonicalName +
            ":" + namenodeInfoPort +
            "/dfshealth.jsp\">Go back to DFS home</a>");
    dfs.close();
  }
  ...
  static void generateFileChunks(JspWriter out, HttpServletRequest req,
    ...
    final String tokenString =
        req.getParameter(JspHelper.DELEGATION_PARAMETER_NAME);
    UserGroupInformation ugi = JspHelper.getUGI(req, conf);
    int namenodeInfoPort = -1;
    if (namenodeInfoPortStr != null) {
      namenodeInfoPort = Integer.parseInt(namenodeInfoPortStr);
    ...
    final String filename = JspHelper.validatePath(
        StringEscapeUtils.unescapeHtml(req.getParameter("filename")));
    if (filename == null) {
    ...
    out.print("<hr>");
    out.print(
        "<a href=\"" + HttpConfig2.getSchemePrefix() + req.getServerName() +
    ...
    out.print(
        "<a href=\"#viewOptions\">Advanced view/download options</a><br>");
    out.print("<hr>");
      ...
      for (int i = 0; i < blocks.size(); i++) {
        if (blocks.get(i).getBlock().getBlockId() == blockId) {
          if (i != blocks.size() - 1) {
            ...
            nextGenStamp =
                Long.toString(nextBlock.getBlock().getGenerationStamp());
            nextStartOffset = 0;
    ...
    if (nextBlockIdStr != null) {
      nextUrl = HttpConfig2.getSchemePrefix() + canonicalize(nextHost) + ":" +
          nextPort + "/browseBlock.jsp?blockId=" + nextBlockIdStr +
          "&blockSize=" + nextBlockSize + "&startOffset=" + nextStartOffset +
          "&genstamp=" + nextGenStamp + "&filename=" +
          URLEncoder.encode(filename, "UTF-8") + "&chunkSizeToView=" +
          chunkSizeToView + "&datanodePort=" + nextDatanodePort +
          "&namenodeInfoPort=" + namenodeInfoPort +
          JspHelper.getDelegationTokenUrlParam(tokenString) +
          JspHelper.getUrlParam(JspHelper.NAMENODE_ADDRESS, nnAddr);
      ...
      for (int i = 0; i < blocks.size(); i++) {
        if (blocks.get(i).getBlock().getBlockId() == blockId) {
          if (i != 0) {
            ...
            prevGenStamp =
                Long.toString(prevBlock.getBlock().getGenerationStamp());
            prevStartOffset =
                prevBlock.getBlock().getNumBytes() - chunkSizeToView;
    ...
    if (prevBlockIdStr != null) {
      prevUrl = HttpConfig2.getSchemePrefix() + canonicalize(prevHost) + ":" +
          prevPort + "/browseBlock.jsp?blockId=" + prevBlockIdStr +
          "&blockSize=" + prevBlockSize + "&startOffset=" + prevStartOffset +
          "&filename=" + URLEncoder.encode(filename, "UTF-8") +
          "&chunkSizeToView=" + chunkSizeToView + "&genstamp=" + prevGenStamp +
          "&datanodePort=" + prevDatanodePort + "&namenodeInfoPort=" +
    ...
    try {
      JspHelper.streamBlockInAscii(
          ...
          blockId, blockToken, genStamp, blockSize, startOffset,
          chunkSizeToView, out, conf, dfs.getDataEncryptionKey());
    } catch (Exception e) {
      out.print(e);
    }
    out.print("</textarea>");
  ...
  static void generateFileChunksForTail(JspWriter out, HttpServletRequest req,
    ...
    String namenodeInfoPortStr = req.getParameter("namenodeInfoPort");
    String nnAddr = StringEscapeUtils
        .escapeHtml(req.getParameter(JspHelper.NAMENODE_ADDRESS));
    int namenodeInfoPort = -1;
    if (namenodeInfoPortStr != null) {
      namenodeInfoPort = Integer.parseInt(namenodeInfoPortStr);
    ...
    out.print("<input type=\"text\" name=\"chunkSizeToView\" value=" +
        chunkSizeToView + " size=10 maxlength=10>");
    out.print(
        "&nbsp;&nbsp;<input type=\"submit\" name=\"submit\" value=\"Refresh\"><hr>");
    out.print(
        "<input type=\"hidden\" name=\"filename\" value=\"" + filename + "\">");
    out.print("<input type=\"hidden\" name=\"namenodeInfoPort\" value=\"" +
        namenodeInfoPort + "\">");
    ...
    final long startOffset =
        blockSize >= chunkSizeToView ? blockSize - chunkSizeToView : 0;
