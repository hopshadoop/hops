...
public class HftpFileSystem extends FileSystem
  ...
  protected InetSocketAddress getNamenodeSecureAddr(URI uri) {
    ...
    return NetUtils
        .createSocketAddrForHost(uri.getHost(), getDefaultSecurePort());
  }
  ...
  protected Token<DelegationTokenIdentifier> selectDelegationToken(
    ...
    return hftpTokenSelector
        .selectToken(nnSecureUri, ugi.getTokens(), getConf());
  }
      ...
      return ugi.doAs(new PrivilegedExceptionAction<Token<?>>() {
        ...
        public Token<?> run() throws IOException {
          ...
          for (Token<? extends TokenIdentifier> t : c.getAllTokens()) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  "Got dt for " + getUri() + ";t.service=" + t.getService());
            }
            return t;
          }
          return null;
  ...
  protected URL getNamenodeURL(String path, String query) throws IOException {
    final URL url =
        new URL("http", nnUri.getHost(), nnUri.getPort(), path + '?' + query);
  ...
  class LsParser extends DefaultHandler {
    ...
    public void startElement(String ns, String localname, String qname,
      ...
      FileStatus fs = "file".equals(qname) ?
          ...
          new FileStatus(0L, true, 0, 0L, modif, atime,
              FsPermission.valueOf(attrs.getValue("permission")),
              attrs.getValue("owner"), attrs.getValue("group"),
              HftpFileSystem.this.makeQualified(
                  new Path(getUri().toString(), attrs.getValue("path"))));
      fslist.add(fs);
    ...
    private void fetchList(String path, boolean recur) throws IOException {
      try {
        ...
        HttpURLConnection connection =
            openConnection("/listPaths" + ServletUtil.encodePath(path),
    ...
    public FileStatus[] listStatus(Path f, boolean recur) throws IOException {
      ...
      if (fslist.size() > 0 &&
          (fslist.size() != 1 || fslist.get(0).isDirectory())) {
        fslist.remove(0);
      }
  ...
  private class ChecksumParser extends DefaultHandler {
    ...
    private FileChecksum getFileChecksum(String f) throws IOException {
      final HttpURLConnection connection =
          openConnection("/fileChecksum" + ServletUtil.encodePath(f),
              "ugi=" + getEncodedUgiParameter());
      try {
  ...
  private class ContentSummaryParser extends DefaultHandler {
    ...
    private ContentSummary getContentSummary(String path) throws IOException {
      final HttpURLConnection connection =
          openConnection("/contentSummary" + ServletUtil.encodePath(path),
              "ugi=" + getEncodedUgiParameter());
      InputStream in = null;
      try {
  ...
  private static ContentSummary toContentSummary(Attributes attrs)
    ...
    if (length == null || fileCount == null || directoryCount == null ||
        quota == null || spaceConsumed == null || spaceQuota == null) {
      return null;
    }

    try {
      return new ContentSummary(Long.parseLong(length),
          Long.parseLong(fileCount), Long.parseLong(directoryCount),
          Long.parseLong(quota), Long.parseLong(spaceConsumed),
          Long.parseLong(spaceQuota));
    } catch (Exception e) {
      throw new SAXException(
          "Invalid attributes: length=" + length + ", fileCount=" + fileCount +
              ", directoryCount=" + directoryCount + ", quota=" + quota +
