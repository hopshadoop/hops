...
public class Check {
  ...
  public static <T> List<T> notNullElements(List<T> list, String name) {
    ...
    for (int i = 0; i < list.size(); i++) {
      notNull(list.get(i),
          MessageFormat.format("list [{0}] element [{1}]", name, i));
    }
    return list;
  }
  ...
  public static String validIdentifier(String value, int maxLen, String name) {
    ...
    if (value.length() > maxLen) {
      throw new IllegalArgumentException(MessageFormat
          .format("[{0}] = [{1}] exceeds max len [{2}]", name, value, maxLen));
    }
    if (!IDENTIFIER_PATTERN.matcher(value).find()) {
      throw new IllegalArgumentException(MessageFormat
          .format("[{0}] = [{1}] must be '{2}'", name, value,
   ...
   * @return the value.
   * @throws IllegalArgumentException
   *     if the integer is zero or less.
   */
  ...
  public static long gt0(long value, String name) {
    if (value <= 0) {
      throw new IllegalArgumentException(MessageFormat
          .format("parameter [{0}] = [{1}] must be greater than zero", name,
              value));
    }
    return value;
  }
   ...
   * @return the value.
   * @throws IllegalArgumentException
   *     if the integer is greater or equal to zero.
   */
  ...
  public static long ge0(long value, String name) {
    if (value < 0) {
      throw new IllegalArgumentException(MessageFormat
          .format("parameter [{0}] = [{1}] must be greater than or equals zero",
              name, value));
    }
    return value;
  }
