 ...
 * <p/>
 * The <code>HttpFSServer</code> class uses Jersey JAX-RS to binds HTTP requests
 * to the
...
public class HttpFSServer {
  ...
  /**
   * Resolves the effective user that will be used to request a FileSystemAccess
   * filesystem.
   * <p/>
   * If the doAs-user is NULL or the same as the user, it returns the user.
   * <p/>
   * Otherwise it uses proxyuser rules (see {@link ProxyUser} to determine if
   * the
   ...
   *     thrown if the current user cannot impersonate
   *     the doAs-user.
   */
  private String getEffectiveUser(Principal user, String doAs)
      throws IOException {
   ...
   */
  private <T> T fsExecute(Principal user, String doAs,
  ...
  private void enforceRootPath(HttpFSFileSystem.Operation op, String path) {
    if (!path.equals("/")) {
      throw new UnsupportedOperationException(MessageFormat
          .format("Operation [{0}], invalid path [{1}], must be '/'", op,
              path));
    }
  }
  ...
  public Response get(
    ...
    switch (op.value()) {
      case OPEN: {
        ...
        AUDIT_LOG
            .info("[{}] offset [{}] len [{}]", new Object[]{path, offset, len});
      ...
      case LISTSTATUS: {
        ...
        Map json = fsExecute(user, doAs, command);
        AUDIT_LOG
            .info("[{}] filter [{}]", path, (filter != null) ? filter : "-");
        response = Response.ok(json).type(MediaType.APPLICATION_JSON).build();
        break;
      }
      ...
      default: {
        throw new IOException(MessageFormat
            .format("Invalid HTTP GET operation [{0}]", op.value()));
      }
    }
    return response;
  }
  ...
  public Response post(InputStream is,
    ...
    switch (op.value()) {
      ...
      default: {
        throw new IOException(MessageFormat
            .format("Invalid HTTP POST operation [{0}]", op.value()));
      }
    }
    return response;
  }
  ...
  protected URI createUploadRedirectionURL(UriInfo uriInfo,
    ...
    uriBuilder =
        uriBuilder.replaceQueryParam(OperationParam.NAME, uploadOperation).
